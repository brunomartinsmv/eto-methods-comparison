...
**PT**
Pipeline Python para reprodutibilidade. O comando unico recomendado e `quickstart`. Etapas individuais tambem existem: `clean`, `compute-eto`, `calibrate`, `aggregate`, `metrics`, `plots`, `pca`, `validate-data`, `analyze-uncertainty`, `summarize`, `export-supplement` e `all`. Os diagramas de Taylor (diario e mensal) sao gerados em `plots`.

**EN**
Python pipeline for reproducibility. The recommended single entry point is `quickstart`. Individual steps also exist: `clean`, `compute-eto`, `calibrate`, `aggregate`, `metrics`, `plots`, `pca`, `validate-data`, `analyze-uncertainty`, `summarize`, `export-supplement`, and `all`. Taylor diagrams (daily and monthly) are generated by `plots`.
