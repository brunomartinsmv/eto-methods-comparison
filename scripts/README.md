...
**PT**
Pipeline Python para reprodutibilidade. O CLI organiza as etapas em comandos claros: `clean`, `compute-eto`, `aggregate`, `metrics`, `plots`, `pca`, `validate-data`, `analyze-uncertainty`, `summarize`, `reproduce-paper`, `export-supplement` e `all`. Os diagramas de Taylor (diario e mensal) sao gerados em `plots`.

**EN**
Python pipeline for reproducibility. The CLI exposes clear steps: `clean`, `compute-eto`, `aggregate`, `metrics`, `plots`, `pca`, `validate-data`, `analyze-uncertainty`, `summarize`, `reproduce-paper`, `export-supplement`, and `all`. Taylor diagrams (daily and monthly) are generated by `plots`.
