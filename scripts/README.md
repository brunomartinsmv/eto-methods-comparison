...
**PT**
Pipeline Python para reprodutibilidade. O CLI organiza as etapas em comandos claros: `clean`, `aggregate`, `metrics`, `plots`, `validate-data`, `summarize`, `reproduce-paper`, `export-supplement` e `all`. Os diagramas de Taylor (diario e mensal) sao gerados em `plots`.

**EN**
Python pipeline for reproducibility. The CLI exposes clear steps: `clean`, `aggregate`, `metrics`, `plots`, `validate-data`, `summarize`, `reproduce-paper`, `export-supplement`, and `all`. Taylor diagrams (daily and monthly) are generated by `plots`.
